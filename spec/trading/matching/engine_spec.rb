...
describe Matching::Engine do
  ...
  context 'submit market order' do
    ...
    it 'should fill the market order completely' do
      ...
      expect(AMQP::Queue).to receive(:enqueue).with(:trade_executor, { action: "execute", trade: { market_id: market.id, maker_order_id: ask1.id, taker_order_id: mo.id, strike_price: ask1.price, amount: ask1.volume, total: '1.0'.to_d } }, anything)
      expect(AMQP::Queue).to receive(:enqueue).with(:trade_executor, { action: "execute", trade: { market_id: market.id, maker_order_id: ask2.id, taker_order_id: mo.id, strike_price: ask2.price, amount: ask2.volume, total: '2.0'.to_d } }, anything)
      expect(AMQP::Queue).to receive(:enqueue).with(:trade_executor, { action: "execute", trade: { market_id: market.id, maker_order_id: ask3.id, taker_order_id: mo.id, strike_price: ask3.price, amount: '0.4'.to_d, total: '1.2'.to_d } }, anything)
    ...
    it 'should fill the market order partially and cancel it' do
      ...
      expect(AMQP::Queue).to receive(:enqueue).with(:trade_executor, { action: "execute", trade: { market_id: market.id, maker_order_id: ask1.id, taker_order_id: mo.id, strike_price: ask1.price, amount: ask1.volume, total: '1.0'.to_d } }, anything)
      expect(AMQP::Queue).to receive(:enqueue).with(:trade_executor, { action: "execute", trade: { market_id: market.id, maker_order_id: ask2.id, taker_order_id: mo.id, strike_price: ask2.price, amount: ask2.volume, total: '2.0'.to_d } }, anything)
      expect(AMQP::Queue).to receive(:enqueue).with(:trade_executor, hash_including(action: 'cancel', order: hash_including(id: mo.id)), anything)
  ...
  context 'submit limit order' do
    ...
    context 'match order with many counter orders' do
      ...
      it 'should execute trade' do
        expect(AMQP::Queue).to receive(:enqueue).exactly(asks.size).times
  ...
  context 'dryrun' do
    ...
    it 'should not publish matched trades' do
      expect(AMQP::Queue).to receive(:enqueue).never
  ...
  context 'publish_increment' do
    ...
    it 'should publish increment of orderbook' do
      expect(AMQP::Queue).to receive(:enqueue_event).with("public", market.id, "ob-inc", { "asks" => ["10.0", "5.0"], "sequence" => 2, })
      expect(AMQP::Queue).to receive(:enqueue_event).with("public", market.id, "ob-inc", { "bids" => ["10.0", "5.0"], "sequence" => 3, })
  ...
  context 'publish_snapshot' do
    ...
    it 'should publish snapshot of orderbook' do
      ...
      expect(AMQP::Queue).to receive(:enqueue_event).with("public", market.id, "ob-snap", {
    ...
    context 'periodic publish snapshot time' do
      ...
      it 'should publish snapshot of orderbook and set increment_count to 1' do
        ...
        expect(AMQP::Queue).to receive(:enqueue_event).with("public", market.id, "ob-inc", { "asks" => ["11.0", "2.0"], "sequence" => 2 })
